...
def inOutVO(v_test, vo):
    ...
    for i in range(len(vo)-1):
        if not np.any(v_test):
            break
        v_out, v_test  = (orientation(vo[i,:], vo[i+1,:], v_test))
        if np.any(v_out):
            v_outn = np.vstack((v_outn, v_out))
    return np.array(v_outn), np.array(v_test)

# @profile
def check_collision(vel_p, vel_obs):
    """ Function Check for Collision """
    
    vel_p_xy = vect_to_xy(vel_p)
    vel_p_xy = np.reshape(vel_p_xy, (1,2))
    outo, intu = inOutVO(vel_p_xy, vel_obs)
...
def check_collision_col(vel_point, pos_tip, col_cons):
    """ Function Check for Collision """
    ...
    vel_point_xy = vect_to_xy(vel_point)
    vect_check_xy = vel_point_xy - pos_tip
    vect_check = vect_to_ang_mag(vect_check_xy)
    
    if check_between_angles(vect_check[1],bound_left,bound_right):
        collision = True
    else:
        collision = False
    return collision

def check_between_angles(angle, bound_left, bound_right):
...
def calc_vel_obst(TS, ang_os_rad):
    ...
    # vel_Relxy = vect_to_xy(vel_Rel) # (Just for plotting)
    ...
    vert_VO_unc_no_TTC = add_uncert(unc_speed, unc_angle, vert_VO_no_unc_ttc, pos_TS_rel, vel_TS) # (Just for plotting)
    ...
    if plotting:
        ...
        # plt.annotate('OS', xy=(0,0), xytext=(len_OS/2,-1.1))
...
def calc_colreg_con(vel_OS, vel_TS, vel_TS_xy, pos_TS_rel):
...
def calc_free_vel_space(velobs, vel_space_free):
    ...
    for vo in velobs:
        if np.any(vo):
            vel_space_free, into = inOutVO(vel_space_free, vo)
            if np.any(into):
                in_all = np.vstack((in_all, into))
    
    return vel_space_free, in_all

def calc_new_vel_colreg(vel_des, search_area, vel_OS):
    ...
    if np.any(search_area):
        ...
        new_vel = vect_to_ang_mag(new_vel_xy)
...
def calc_vel_final(ts_info, os_info, this, pos_os):
    global plotting
    plotting = this
    global pos_OS_xy
    pos_OS_xy = pos_os
    ...
    for TSv in TS_VO:
        ...
        vel_TS_xy = vect_to_xy(vel_TS_ang)
        vel_rela = calc_rel_vel(vel_OS, vel_TS_ang)
        vel_rela_xy = vect_to_xy(vel_rela)
    ...
    if True:    
        ...
        th = np.arange(0, 2*np.pi, np.deg2rad(res_ang))
        vel = np.arange(0.000000000001, max_speed_OS+res_speed, res_speed)
        ...
        free_vel, in_all = calc_free_vel_space(TS_VO_check[:,5], v_sample)
        free_vel, col_con = calc_free_vel_space(TS_VO_check[:,11], free_vel)
        ...
        #     plt.scatter(col_con[:,0]+pos_OS_xy[0],col_con[:,1]+pos_OS_xy[1], marker=".", s = 2, c="purple",zorder=5)
            ...
            # free_vel_1 = np.apply_along_axis(vect_to_ang_mag, 1, free_vel)
            # in_all_1 = np.apply_along_axis(vect_to_ang_mag, 1, in_all)
            # col_con_1 = np.apply_along_axis(vect_to_ang_mag, 1, col_con)
    ...
    if np.any(new_vel_testo):
        ...
        for vel_obs in TS_VO_check[:,5]:
            if check_collision(vel_des, vel_obs):
